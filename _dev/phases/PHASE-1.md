...
**Son Güncelleme:** 2026-05-29 — TASK-1.12 ✅ (mobile @sentry/react-native 8.13 + crypto-js 4.2 kuruldu; backend kontratıyla birebir paralel 3 katmanlı KVKK savunma — Sentry `sendDefaultPii: false` + `beforeSend` recursive scrubber (event.request.{data,cookies,query_string,headers} + event.user (id crypto-js sha256-12 hash — Node parity sabit `73475cb40a56`, email/username/ip_address sil) + event.extra + event.contexts + event.breadcrumbs[].data) + `beforeBreadcrumb` URL sanitize + HTTP body drop; `sanitizeUrl()` 8 SENSITIVE_PATH_PATTERNS (`/me/measurements`, `/me/food-log`, `/me/notes`, `/pt/members`, `/members`, `/measurements`, `/food-log`, `/invites`) + defansif `/\/(\d+|UUID|[a-z0-9]{16,})(?=[/?#]|$)/gi` lookahead-anchored (aksi halde `\d+` UUID'nin ilk hex chunk'ını yiyor — test'le bulundu, fix uygulandı); `initSentry()` DSN yoksa no-op + production/staging warn (degrade mode) + `initSentryFromEnv()` `EXPO_PUBLIC_SENTRY_DSN` / `EXPO_PUBLIC_APP_ENV` / `EXPO_PUBLIC_SENTRY_RELEASE` okur; `app.config.ts` `plugins: [..., '@sentry/react-native/expo']` (Yakın 5 EAS Build source map upload için yapı kurulu — auth token yokken no-op); `app/_layout.tsx` module-level `initSentryFromEnv()` + `export default Sentry.wrap(RootLayout)` no-op error boundary; PII_FIELDS SSOT `@alpfit/shared` paylaşımı drift'siz; `sentry.test.ts` 23 PASS — `jest.mock('@sentry/react-native')` ile network/global state riski yok, sentryBeforeSend × 3 + scrubPii × 2 + hashUserId × 4 (Node parity dahil) + sanitizeUrl × 4 + sentryBeforeBreadcrumb × 3 + initSentry × 4 + negatif kanıt; recursive typecheck + lint + format + backend 20 + shared 41 PASS regression yok; manuel staging Sentry dashboard smoke Yakın 1 son task'inde uçtan uca smoke'a ertelendi; sıradaki TASK-1.13 3 rol veri modeli — Member + Trainer + Gym Owner Prisma enum + ilişki tabloları).
