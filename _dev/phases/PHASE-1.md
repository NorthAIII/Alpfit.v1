...
**Son Güncelleme:** 2026-05-29 — TASK-1.14 ✅ (KVKK consent + audit log şeması: 3 yeni enum (`ConsentType`, `ConsentEventType`, `AuditEventType` 16 v1 event) + 2 yeni tablo `ConsentRecord` (versiyonlu append-only, `textVersion` tarih-bazlı + opsiyonel `ipAddress`/`userAgent` KVKK denetim için bilinçli, User FK + index `userId+consentType`/`occurredAt`) + `AuditLog` (`userIdHash` **sha256 prefix 12 hex** — ham userId DB'de YOK, `eventType`, `metadata Json?` zod whitelist enforce; index `userIdHash`/`occurredAt`/`eventType`); `User.consentRecords` relation eklendi; `User.kvkkConsentAt`/`healthConsentAt` denormalized cache olarak kalır, truth source ConsentRecord; migration `20260529205040_kvkk_consent_audit` saf Prisma DDL (raw SQL gereksiz — partial index yok); `backend/src/kvkk/consent.ts` `recordConsent(prisma, args)` Prisma `$transaction` ile (a) event insert + (b) `kvkk_aydinlatma`/`saglik_verisi` için User cache senkron (`pazarlama_iletisim` v1'de cache yok) + `getActiveConsent(prisma, userId, type)` en son event `granted` mı; `backend/src/kvkk/audit.ts` `AuditMetadataSchema` zod **`.strict()` whitelist** 10 izinli alan (`ip`/`deviceType`/`userAgent`/`invitationId`/`refreshTokenId`/`consentType`/`textVersion`/`attemptCount`/`count`/`reason`) → bilinmeyen anahtar ZodError + `logAuditEvent` `hashUserId` pii-scrubber re-use (Sentry event correlation ile aynı algoritma); `shared/src/pii-fields.ts` PII_FIELDS'a `ip`/`ipAddress`/`ip_address`/`userAgent`/`user_agent` eklendi (KVKK IP audit nüansı: DB'ye bilinçli yazılır + AuditLog metadata whitelist izinli ama log/Sentry yoluna sızarsa redaktedir); `audit.test.ts` 8 PASS + `consent.test.ts` 4 PASS = 12 yeni test — audit: PII zod reddi (phone/weight/firstName/email) + DB'ye yazılmama kanıtı + whitelist kabul + hash 12-hex + correlation; consent: granted→active + revoked→inactive append-only + denormalized cache senkron (kvkkConsentAt/healthConsentAt set+null+auto_revoked) + pazarlama cache YOK ama getActiveConsent doğru; backend 38 PASS (önceki 26 + yeni 12), shared 41 + mobile 23 regresyon yok; typecheck + lint + format temiz; DECISIONS.md "KVKK Consent Versiyonlu + AuditLog Whitelist Metadata + UserIdHash" kararı eklendi; KVKK.md schema referansı + audit policy notu eklendi; memory `kvkk-pii-scrubbing-matrisi.md`'ye IP audit nüansı eklendi.
