...
**Son Güncelleme:** 2026-05-30 — TASK-1.16 ✅ (Backblaze B2 off-site yedek + restore drill prosedürü dokümantasyon teslim edildi; Coolify-bağımsız mekanizma TASK-1.10 sapmasıyla hizalı: host VPS crontab + `docker compose exec pg_dump --format=custom --single-transaction --no-owner --no-privileges --compress=6` + rclone B2 native driver + crypt overlay client-side AES. `_dev/docs/backblaze-setup.md` manuel hesap+EU bucket+lifecycle 30g+scoped key+DPA rehberi; `_dev/docs/staging-pg-backup-cron.md` rclone install+non-interaktif config+`/usr/local/bin/alpfit-pg-backup.sh` template+crontab `0 2 * * *` UTC (TR 05:00, retention purge'den 2 saat sonra)+local 7g buffer+logrotate haftalık×8+sanity guard <1KB exit 2; `_dev/docs/restore-drill.md` 7-adım drill prosedürü (rclone indir+PGDMP magic byte sanity → ayrı `restore_test` DB → `pg_restore --exit-on-error --verbose` elapsed → smoke query `\dt`/User count/AuditLog count+MAX(occurredAt)/`_prisma_migrations` → temizlik+drill kaydı). `_dev/memory/restore-drill-disiplini.md` her ayın 15'i süreç disiplini (drift sinyalleri+`/devflow:quick` aksilik akışı+faz review-phase kontrolü), `_dev/memory/staging-infra.md` "B2 Off-Site Yedek" tablosu (provider/region/endpoint/bucket/key+encryption pointer'ları/lifecycle/local buffer/cron/script/log/DPA tarihi — TBD alanları kullanıcı follow-up'da doldurulacak) + "Restore Drill Kayıtları" boş başlangıç + TODO 3 follow-up'a bölündü, MEMORY.md index pointer + son güncelleme satırı; KVKK.md "Üçüncü Taraf Sözleşmeler" listesinde B2 region kararı ✅ (eu-central-003 SCC+DPA savunulabilir, mekanizma özeti, ~$0.02/ay maliyet) + ayrı DPA TODO; DECISIONS.md "2026-05-30 — TASK-1.16: Backblaze B2 EU Off-Site Yedek + rclone Crypt Overlay + Host Crontab + Aylık Drill" karar (5 seçenek matrisi provider/mekanizma/encryption/lifecycle/drill sıklığı + 8 tamamlayıcı + 4 ILKELER gerekçesi + 6 risk-mitigation + 6 kullanıcı follow-up manuel adımı). AskUserQuestion 3 karar: (1) script + host crontab (önerilen, TASK-1.15 paterniyle hizalı), (2) B2 hesabı henüz hazır değil → dokümantasyon+template ile teslim, (3) ilk drill sonraya ertelendi → kullanıcı kendisi yapacak ve `staging-infra.md`'ye yazacak. Test paketleri regresyon: backend 52 + shared 41 + mobile 23 PASS, `pnpm typecheck`+`pnpm lint`+`pnpm format:check` temiz; `bash -n` script blokları (backup script 59 satır + rclone config snippet + drill snippet) syntax temiz; shellcheck dev container'da yüklü değil — bash -n sanity'ı yapıldı. Follow-up'lar (kullanıcı tarafından, repo dışı): Backblaze hesap+bucket+lifecycle, application key + encryption password üretimi + password manager + DPA, sunucuda rclone install+config+script deploy+crontab+logrotate, ilk restore drill — sonuçlar `_dev/memory/staging-infra.md`'ye yazılacak.) (Soft delete + 30 gün retention purge altyapısı: `backend/src/kvkk/soft-delete.ts` üç giriş noktası `softDeleteUser(userId, reason?)` → `User.deletedAt = now` + `retentionDeadline = now + 30g` + AuditLog `member_removed` `$transaction`'da atomik; `endTrainerMember(relationId, reason?)` → `TrainerMember.endedAt = now` + member `retentionDeadline` (sadece sağlık-purge, `deletedAt` SET EDİLMEZ → hesap kalır) + AuditLog `member_removed`; `revokeHealthConsent(userId, textVersion, ipAddress?, userAgent?)` → ConsentRecord `saglik_verisi/revoked` append-only + `User.healthConsentAt = null` + `retentionDeadline` + AuditLog `consent_revoked` whitelist metadata. `RETENTION_DAYS = 30` SSOT. `backend/src/kvkk/retention-job.ts` `runRetentionPurge` deadline geçen User'ları her biri kendi `$transaction`'ında işler (cron sağlamlığı), `deletedAt IS NOT NULL` → ANONİMİZE (firstName/lastName=''/profilePhotoUrl/gymName/certificateNote=null + `phoneE164='deleted_<sha256(userId).slice(0,12)>'` — global unique constraint için collision-safe, `+` taşımayan değer canlı E.164 ile çakışmaz; `deletedAt` korunur denetim kanıtı), aksi → sadece `retentionDeadline = null` reset (sağlık-purge yolu, hesap kalır). `purgeDeletableTablesForUser(_tx, _userId)` v1'de boş imza (Yakın 4'te `measurement.deleteMany({where: {userId}})` + `foodLog.deleteMany` eklenecek; underscore-prefix konvansiyonu TS noUnusedParameters geçer). Toplu `AuditLog.retention_purge` event `userId='retention-job'` sentinel hash (broad disclosure yok) + `count` + `reason` whitelist metadata. `backend/src/routes/admin-internal.ts` (YENİ) + `server.ts` register: `POST /admin/internal/retention-purge` Bearer auth 503 (env yok) / 401 (header eksik/yanlış/Bearer prefix yok) / 200 `{status, report}`; **endpoint internet'e açık DEĞİL** — bunker-nginx server block'unda `/admin/internal/` proxy edilmedi (KVKK saldırı yüzeyi 0), tek erişim yolu container içinden `docker compose exec`. `logAuditEvent` imzası `AuditLogClient = Pick<PrismaClient, 'auditLog'>` yapısal tipe genişledi (tx + full PrismaClient ikisi de geçer; eski TASK-1.14 testleri değişmeden geçer). `config/env.ts` `ADMIN_INTERNAL_TOKEN: z.string().min(32).optional()` + `.env.example` + `_ops/staging/.env.staging.example` satır eklendi (`openssl rand -hex 32` üretim). `_dev/docs/staging-retention-cron.md` (YENİ) host VPS crontab (deploy user) → `/usr/local/bin/alpfit-retention-purge.sh` → `docker compose exec alpfit-backend curl` deseniyle Coolify-bağımsız tetikleme (TASK-1.10 mimari sapma uyumlu): token üretim + .env.staging insert + container restart smoke + script exit code/log/logrotate + `0 0 * * *` UTC = 03:00 TR + manuel test + rollback + "bunker-nginx /admin/internal/ proxy etme" disiplini. `_dev/KVKK.md` "Veri Saklama Politikası" bölümü (3 akış tablosu + anonimizasyon stratejisi + audit retention TODO + host crontab tetikleme + rehber link). `_dev/docs/DECISIONS.md` "2026-05-30 — TASK-1.15: 30 Gün Retention Purge + Anonimizasyon vs Hard Delete + Host Crontab Tetikleme" kararı (3 ana karar + 7 tamamlayıcı + 4 risk-mitigation). Memory `_dev/memory/feedback-snapshot-tarih-pin.md` (YENİ) süreç disiplini — UI snapshot testlerinde tarih/zaman içeren render'lar `jest.useFakeTimers().setSystemTime(...)` ile pin'lenmeli. `backend/src/kvkk/retention-job.test.ts` 14 PASS — 3 soft-delete davranışı (cache + AuditLog + metadata) + 5 runRetentionPurge senaryosu (deadline-geçmemiş-skip + anonimize+AuditLog + sadece-reset + v1.5-ready-boş + idempotent) + 1 env-yok 503 + 5 endpoint senaryosu (401 üç çeşit + 200 boş + 200 gerçek anonimize). Yan fix mobile `landing-screen.test.tsx` snapshot drift (pre-existing) `jest.useFakeTimers().setSystemTime(2026-05-29T12:00:00Z)` ile pin'lendi. Backend 52 PASS (önceki 38 + 14), shared 41 + mobile 23 PASS (snapshot fix sonrası) regresyon yok; typecheck + lint + format temiz. Karar noktaları (anonimize / host crontab / sadece rehber-endpoint + yan snapshot fix) AskUserQuestion ile netleşti.
