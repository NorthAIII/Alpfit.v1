...
**Son Güncelleme:** 2026-05-29 — TASK-1.13 ✅ (Prisma 7 schema 3 rol veri modeli: `Role` enum (`member`/`trainer`/`gym_owner`) + `User` (cuid id, globally unique `phoneE164`, `firstName/lastName`, opsiyonel `gymName/certificateNote/profilePhotoUrl`, `kvkkConsentAt/healthConsentAt`, soft-delete `deletedAt/retentionDeadline`) + `TrainerMember` (trainerId+memberId, startedAt+endedAt nullable) + `GymOwnerTrainer` (v1 boş slot, aynı yapı); migration `20260529190917_three_role_data_model` Prisma'nın ürettiği DDL + **raw partial unique index'ler** — `CREATE UNIQUE INDEX "TrainerMember_memberId_active_unique" ON "TrainerMember"("memberId") WHERE "endedAt" IS NULL` + GymOwnerTrainer için trainer-bazlı eşi; Prisma DSL `@@unique([..., endedAt])` PostgreSQL NULL semantiği yüzünden aktif çoklu satırları engelleyemediğinden raw SQL şart; `relations.ts` `assertSingleActivePtForMember(prisma, memberId)` + `ActiveTrainerRelationExistsError` placeholder helper (TASK-1.24 davet kabul akışında kullanılacak — DB partial index son güvence, helper okunaklı hata mesajı); `shared/src/pii-fields.ts` PII_FIELDS'a `gymName/gym_name`, `certificateNote/certificate_note`, `phoneE164/phone_e164` eklendi (camelCase + snake_case SSOT disiplini); `relations.test.ts` 6 test PASS — phoneE164 global unique (aynı telefon iki rolde reddedilir), gym_owner DB-izinli (UI engellemesi sonraki katman), TrainerMember partial index iki aktif rejekte, ilişki sonlandırıldıktan sonra yeni PT atanabiliyor, `assertSingleActivePtForMember` null+throw matrisi, GymOwnerTrainer partial index iki aktif rejekte; backend 26 PASS (önceki 20 + yeni 6), shared 41 + mobile 23 regression yok; typecheck + lint + format temiz; manuel `psql \d` ile `WHERE "endedAt" IS NULL` partial index doğrulandı.
