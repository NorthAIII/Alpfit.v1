...
| 1.24 | TASK-1.24 | ✅ Tamamlandı | Davet kabul + preview endpoints — POST /invitations/:code/accept (auth, tek-kullanım atomik compare-and-set, own_invitation 400 role 403 öncesi, already_has_trainer 409, soft-deleted PT 404, audit invitation_accepted) + GET /invitations/:code (public preview, PT ad/soyad) + relations.ts helper'lar (getActivePtForMember/assertNoActivePt/createPtMemberRelation) |
| 1.25 | TASK-1.25 | ✅ Tamamlandı | Deep link (Universal/App Link + .well-known/) — backend `.well-known/` route (AASA application/json + assetlinks, env APPLE_APP_ID/ANDROID_SHA256 placeholder) + `/davet/:code` masaüstü fallback (sunucu-taraflı inline QR, `qrcode` paketi); mobile app.config deep link (alpfit-staging.kiwiailab.com + alpfit.app) + `app/davet/[code].tsx` preview ekranı + davet i18n; bunker-nginx proxy → nginx'e dokunulmadı; auth-state dallanma TASK-1.26+'ya ertelendi |
| 1.26 | TASK-1.26 | ✅ Tamamlandı | Açılış ekranı (rol seçimi + manuel davet kodu + deep link) — `app/index.tsx` üç buton (Üyeyim/Antrenörüm/Davetim var) + inline davet kodu (trUpper, 6 char, ABC-123 mask, preview→navigate/inline hata); onboarding store (zustand 5, kullanıcı onaylı); `app/davet/[code].tsx` "Devam et" store yazıp `/auth/phone`'a bypass; i18n common.role.*/landing.*/errors.invitation_invalid; mobile 50 PASS |
| 1.27 | TASK-1.27 | ✅ Tamamlandı | Telefon girişi ekranı (+90 inline validation) — `app/auth/phone.tsx` sabit +90 ön ek + 10 hane ulusal mask (5XX XXX XX XX), inline doğrulama 50ms debounce (kırmızı/yeşil border), Devam → `setPhone`+`sendOtp` (POST /auth/otp/send) → /auth/otp; 429 60s countdown, network PII'siz Sentry; yeni/mevcut ayrım YAPILMAZ (F1.1); `src/auth/phone-mask.ts` + `src/api/auth.ts` (YENİ); i18n auth.phone.*/errors.rate_limit/network; mobile 55 PASS |
| 1.28 | TASK-1.28 | ✅ Tamamlandı | KVKK rıza ekranı (2 tickbox + placeholder metin) — sıra dışı |
| 1.29 | TASK-1.29 | ✅ Tamamlandı | OTP girişi ekranı (timer + yeniden gönder + dev lookup) — `app/auth/otp.tsx` 6 kutu + 3 sayaç (kod 5dk / resend 60sn / kilit 423→15dk), onComplete→`verifyOtp`; yeni→registrationToken store+`/auth/kvkk`, mevcut→`replace('/')` (token persist TASK-1.33); `src/auth/otp-input.tsx` (auto-advance+backspace+paste/oneTimeCode); `src/api/auth.ts` verifyOtp/fetchDevOtp/isDevOtpLookupEnabled; store registrationToken; dev OTP env (EXPO_PUBLIC_DEV_OTP_LOOKUP/_TOKEN); mobile 64 PASS |
| 1.30 | TASK-1.30 | ✅ Tamamlandı | Profil oluşturma ekranı (üye + PT) — `app/auth/profile.tsx` ad+soyad (zod 2-50 + `\p{L}` TR-izinli, blur inline) + PT-only spor salonu/sertifika notu + avatar UI-only; submit→`createProfile` (kayıt jetonu Bearer), `member_via_invite`→`acceptInvitation` ardışık (network 3 deneme/terminal hata uyarısı), diğer→`/home` placeholder; 409→"Giriş yap"→phone. `src/auth/profile-schema.ts`, `src/api/auth.ts` (createProfile/acceptInvitation/ProfileUser), `app/home/index.tsx`, i18n auth.profile.*/common.home.*; mobile 71 PASS |
| 1.31 | TASK-1.31 | ✅ Tamamlandı | PT "Üyeler" sekmesi UI — backend `GET /trainers/me/members` (trainer-only, endedAt:null + member soft-delete hariç, sadece id/ad/joinedAt — telefon DÖNMEZ; 7 test) + mobile `app/(tabs)/members.tsx`+`_layout.tsx` (Bekleyen davetler + Aktif üyeler tek liste, boş→CTA, useFocusEffect + pull-to-refresh), InviteModal (Clipboard) + QrModal (react-native-qrcode-svg); `src/api/trainers.ts`+`invitations.ts` (create/list/cancel) + `src/auth/session.ts` (in-memory store); paketler expo-clipboard/qrcode-svg/svg; gün-kalan `{{days}}`; mobile 77 PASS + backend 7; kalıcı oturum+role-guard TASK-1.33'e ertelendi |
...
**Son Güncelleme:** 2026-05-30 — TASK-1.31 ✅: PT "Üyeler" sekmesi UI. Backend `GET /trainers/me/members` (trainer-only, endedAt:null + member soft-delete hariç, sadece id/ad/joinedAt — telefon DÖNMEZ, KVKK). Mobile `app/(tabs)/members.tsx`+`_layout.tsx` (Bekleyen davetler + Aktif üyeler tek liste, boş→CTA, useFocusEffect + pull-to-refresh), InviteModal (Clipboard) + QrModal (react-native-qrcode-svg). `src/api/trainers.ts`+`invitations.ts` (create/list/cancel) + `src/auth/session.ts` (in-memory store). Paketler: expo-clipboard, react-native-qrcode-svg, react-native-svg. mobile 77 PASS + backend trainers-members 7; typecheck/lint/format temiz. Kalıcı oturum + role-guard TASK-1.33'e ertelendi.
