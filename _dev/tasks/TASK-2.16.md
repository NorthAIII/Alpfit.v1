...
- [ ] **1. `completeWorkout` — programDayId ownership doğrulaması**
...
- [ ] **2. `publishProgram` — status kontrolü**
...
- [ ] **3. `patchProgram` — silinmiş egzersiz kontrolü**
...
- [ ] `POST /workout-completions` — üye kendi aktif programına ait olmayan `programDayId` → 403
- [ ] `POST /programs/:id/publish` — `status='active'` program → 403
- [ ] `POST /programs/:id/publish` — `status='archived'` program → 403
- [ ] `PATCH /programs/:id` — `deletedAt` dolu egzersiz ID içeriyorsa → 422
- [ ] Mevcut 227 backend testi hâlâ geçiyor
