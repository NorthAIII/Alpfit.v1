...
**Yapılanlar:**
- `shared/src/schemas/program.ts`: `hasUnreadUpdate: z.boolean().optional()` eklendi; shared dist yeniden build edildi.
- `mobile/test/mocks/async-storage.ts` (YENİ): In-memory AsyncStorage test mock'u (expo-secure-store pattern'i izlendi). `jest.config.js` moduleNameMapper + `test/setup.ts` afterEach reset güncellendi.
- `mobile/src/stores/memberBannerStore.ts` (YENİ): Session-içi program_changed banner state (Zustand). `programChangedBanner: string | null`, `showBanner / hideBanner / clear`.
- `mobile/src/hooks/useMemberHome.ts`: `useProgramChangedBanner` hook eklendi. `hasUnreadUpdate === true` → AsyncStorage dismiss kaydını kontrol eder → banner store günceller. Dismiss → AsyncStorage'a `{ [programId]: publishedAt }` persist. Yeni publish (publishedAt ilerler) → eski dismiss geçersiz → banner yeniden gösterilir.
- `mobile/app/home/index.tsx`: Banner stack placeholder kaldırıldı; `programChangedBanner` ("ℹ️ Programında güncelleme var") nötr mavi banner + ✕ dismiss. BUGÜN kartı ve WeeklyBand etkilenmedi.
- `mobile/src/stores/memberBannerStore.test.ts` (YENİ): 5 store unit testi.
- `mobile/src/hooks/useProgramChangedBanner.test.ts` (YENİ): 5 hook integration testi (null, false, true, dismiss persist, yeni publish).
- `mobile/app/home/index.test.tsx`: 4 yeni banner UI testi (banner yok, banner var, dismiss, BUGÜN kartı etkilenmez). `useProgramChangedBanner` mock'a eklendi.
- Toplam: 251 mobile test, 0 hata. TypeScript 0 hata.
