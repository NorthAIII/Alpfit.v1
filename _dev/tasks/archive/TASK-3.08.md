...
**Yapılanlar:**
- `notification.service.ts` (YENİ): `sendMorningReminders(prisma, pushChannel)` — late check (>30dk), DB sorgusu (aktif program + bugün antrenman + morningHour eşleşme), per-member sessiz saat / no_token / reminder_disabled skip + log
- `notification.service.test.ts` (YENİ): 7 senaryo (happy path, morningHour mismatch, reminderEnabled:false, no_token, silent hours mock, no workout today, late job)
- `notification.worker.ts` güncellendi: morning-reminder sistem job'a dönüştürüldü (userId yok, sendMorningReminders çağrısı), saatlik `0 * * * *` Istanbul repeatable job kaydedildi, ExpoPushAdapter enjekte edildi
- `queue.test.ts` güncellendi: smoke test QueueEvents + waitUntilFinished ile yeniden yazıldı (morning-reminder artık sistem job)

**Test sonucu:** 36 dosya, 290 test — hepsi yeşil (önceki 282 + 8 yeni)
